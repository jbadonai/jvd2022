...
def merge_v1(audio_list, video_list, video_key, output_directory, segment_folder_name, final_export=0):
    ...
    os.system("ffmpeg -loglevel panic -y -f concat -safe 0 -i \"{}\" -c:a copy \"{}\"".format(
        output_directory / f"list_{video_key}_audio.txt", output_directory / f"{video_key}_audio_v1_ffmpeg.m4a"))
    ...
    os.system("ffmpeg -loglevel panic -y -f concat -safe 0 -i \"{}\" -c:v copy \"{}\"".format(
        output_directory / f"list_{video_key}_video.txt", output_directory / f"{video_key}_video_v1_ffmpeg.mp4"))
    ...
    os.system("ffmpeg -loglevel panic -y -i \"{}\" -i \"{}\" -c:a copy -c:v copy \"{}\"".format(
        output_directory / f"{video_key}_video_v1_ffmpeg.mp4", output_directory / f"{video_key}_audio_v1_ffmpeg.m4a",
...
def merge_v2(audio_list, video_list, video_key, output_directory, segment_folder_name, final_export=0):
    ...
    os.system(
        "ffmpeg -loglevel panic -y -i \"{}\" -c:a copy \"{}\"".format(output_directory / f"concat_{video_key}_audio.ts",
                                                                      output_directory / f"{video_key}_audio_v2_ffmpeg.m4a"))
    ...
    os.system(
        "ffmpeg -loglevel panic -y -i \"{}\" -c:v copy \"{}\"".format(output_directory / f"concat_{video_key}_video.ts",
                                                                      output_directory / f"{video_key}_video_v2_ffmpeg.mp4"))
    ...
    os.system("ffmpeg -loglevel panic -y -i \"{}\" -i \"{}\" -c:a copy -c:v copy \"{}\"".format(
        output_directory / f"{video_key}_audio_v2_ffmpeg.m4a", output_directory / f"{video_key}_video_v2_ffmpeg.mp4",
        output_directory / f"{video_key}_v2.mp4"))
...
for f in range(first_segment, last_segment):
    if not pathlib.Path(output_directory / segment_folder_name / f"{f}_{video_key}_video.ts").is_file():
        print_warning(
            "Missing segment: {}".format(output_directory / segment_folder_name / f"{f}_{video_key}_video.ts"))
        missing_files = True
    elif not pathlib.Path(output_directory / segment_folder_name / f"{f}_{video_key}_audio.ts").is_file():
        print_warning(
            "Missing segment: {}".format(output_directory / segment_folder_name / f"{f}_{video_key}_audio.ts"))
        missing_files = True
    else:
...
cmd = " ".join(
    ['ffprobe', '-v', 'quiet', '-hide_banner', '-show_streams', '\"{}\"'.format(total_segment_list_sorted_video[0]),
     '2>&1'])
...
v1_test_path = merge_v1(merge_v1_test_segments_audio, merge_v1_test_segments_video, video_key, output_directory,
                        segment_folder_name)
...
v2_test_path = merge_v2(merge_v1_test_segments_audio, merge_v1_test_segments_video, video_key, output_directory,
                        segment_folder_name)
...
if f1_working:
    ...
    v1_path = merge_v1(total_segment_list_sorted_audio, total_segment_list_sorted_video, video_key, output_directory,
                       segment_folder_name, 1)
    print("Output file: {}".format(v1_path))
elif f2_working:
    print_info("using method 2 for this livestream. This process might take a while...")
    v2_path = merge_v2(total_segment_list_sorted_audio, total_segment_list_sorted_video, video_key, output_directory,
                       segment_folder_name, 1)
    print("Output file: {}".format(v2_path))
else:
    print_error(
        "Both methods aren't working for some reason. Don't delete the recording just yet! If you're sure that you didn't mess with the files in any way, please open an issue on Github and report this.")
