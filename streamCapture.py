...
print(args)
args.append('https://www.youtube.com/watch?v=BZT8jqsc8lQ&list=RDBZT8jqsc8lQ&start_radio=1&rv=BZT8jqsc8lQ&t=6')
...
def get_segment_list(dash_content, itag):
    ...
    if itag in quality_video_ranking:
        ...
        for i in range(i, 999999):
            try:
                ((dash_content.split("<Representation id=\"{}\"".format(itag))[1]).split("</SegmentList>")[0]).split(
                    "sq/{}/".format(i))[1]
            except:
                break
        last_segment = i
        ...
        for k in (range(0, last_segment)):
            segment_list.append(
                "{}sq/{}/{}".format(video_base_url, k, video_lmt_number - (video_lmt_distance * (last_segment - k))))
    ...
    if itag in quality_audio_ranking:
        ...
        for k in (range(0, last_segment)):
            segment_list.append(
                "{}sq/{}/{}".format(audio_base_url, k, audio_lmt_number - (audio_lmt_distance * (last_segment - k))))
...
def get_new_segment(dash_content, itag, old_segment_number):
    ...
    if itag in quality_audio_ranking:
        return "{}sq/{}/{}".format(audio_base_url, old_segment_number,
                                   audio_lmt_number - (audio_lmt_distance * (old_segment_number)))
    if itag in quality_video_ranking:
        return "{}sq/{}/{}".format(video_base_url, old_segment_number,
                                   video_lmt_number - (video_lmt_distance * (old_segment_number)))
...
def run_script():
    ...
    for i in range(segment_number, 999999):
        ...
        if segment_number < len(video_segment_list):
            ...
            os.system(
                "aria2c -c --auto-file-renaming=false --max-tries=100 --retry-wait=5 -j 3 -x 3 -s 3 -k 1M \"{}\" -d \"{}\" -o \"{}\"".format(
                    video_segment_list[segment_number], output_directory,
                    f"{segment_number}_{filename_thing}_video.ts"))
            while (True):
                try:
            ...
            os.system(
                "aria2c -c --auto-file-renaming=false --max-tries=100 --retry-wait=5 -j 3 -x 3 -s 3 -k 1M \"{}\" -d \"{}\" -o \"{}\"".format(
                    audio_segment_list[segment_number], output_directory,
                    f"{segment_number}_{filename_thing}_audio.ts"))
            try:
                if pathlib.Path(
                        output_directory / f"{segment_number}_{filename_thing}_video.ts").stat().st_size < 2000 or pathlib.Path(
                        output_directory / f"{segment_number}_{filename_thing}_audio.ts").stat().st_size < 2000:
                    segment_number -= 4
                    print("Trying again!")
                    continue
                else:
